...
def recorded_variant(arm):
    """run meta에 기록된 arm의 계약을 다시 만든다. 기록 당시와 hash가 같아야 한다.
    ...
    if (variant.sha256, variant.repair_sha256) == wanted:
        return variant
    for name in sorted(PINNED_SHA256):
        if (PINNED_SHA256[name], PINNED_REPAIR_SHA256.get(name)) == wanted:
            return build_variant(name)
    raise BenchmarkAborted(f"기록된 계약을 다시 만들 수 없다: {arm['variant']} {wanted}")
